...
function botMove() {
          // Create an array to store the indices of empty cells
          const emptyCells = [];
          
          // Iterate through all cells to find the empty ones
          for (let i = 0; i < cells.length; i++) {
              if (cells[i].textContent === '') {
                  emptyCells.push(i); // Add the index of an empty cell to the array
              }
          }
          
          // Generate a random index from the array of empty cells
          const randomIndex = Math.floor(Math.random() * emptyCells.length);
          const index = emptyCells[randomIndex];
          
          // Assign the bot's move (O) to the randomly selected cell
          cells[index].style.backgroundColor = 'lightcoral';
          cells[index].textContent = 'O';
          oMoves.push(index); // Store the bot's move in the oMoves array
      }
